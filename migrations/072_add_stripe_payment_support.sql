...
ALTER TABLE loyers_tracking
    ADD COLUMN IF NOT EXISTS stripe_session_id VARCHAR(255) NULL COMMENT 'Dernière session Stripe associée',
    ADD COLUMN IF NOT EXISTS mode_paiement ENUM('manuel','stripe','virement','especes','cheque') DEFAULT 'manuel' COMMENT 'Mode de paiement utilisé',
    ADD COLUMN IF NOT EXISTS date_paiement TIMESTAMP NULL COMMENT 'Date de confirmation du paiement';
